...
//コンストラクタ
//==========================
CEnemyBeforeActionState::CEnemyBeforeActionState()
{
...
//開始
//==========================
void CEnemyBeforeActionState::Start()
{
	//モーションを設定
...
//更新
//==========================
void CEnemyBeforeActionState::Update()
{
	//ゲームマネージャーの情報
	CGameManager* GameManager = CManager::GetInstance()->GetGameManager();

	//モーションを設定
	m_pEnemy->SetMotion(CMotionModel::MOTION_TYPE::NEUTRAL);

	int ActionWave = m_pEnemy->GetActionWave();
	int Wave = GameManager->GetWave();

	if (m_pEnemy->GetActionWave()== GameManager->GetWave())
	{//現在のウェーブが行動するウェーブと同じ

		//行動してる敵の数を増やす
		GameManager->AddActiveEnemy();
...
//終了
//==========================
void CEnemyBeforeActionState::Uninit()
{
